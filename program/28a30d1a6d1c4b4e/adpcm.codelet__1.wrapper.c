...
int main(int argc, const char **argv)
{
  ...
  char * inp__region_buffer = (char *) __astex_memalloc(2000);
  __astex_write_message("Reading inp value from %s\n", argv[1]);
  __astex_read_from_file(inp__region_buffer, 2000, codelet_data_file_descriptor);
  inp = (short *) (inp__region_buffer + 0l);
  char * outp__region_buffer = (char *) __astex_memalloc(500);
  __astex_write_message("Reading outp value from %s\n", argv[1]);
  __astex_read_from_file(outp__region_buffer, 500, codelet_data_file_descriptor);
  outp = (signed char *) (outp__region_buffer + 0l);
  ...
  {
    --_astex_wrap_loop;
  astex_codelet__1(len, state, indexTable, stepsizeTable, inp, outp, step, valpred, index, outputbuffer, bufferstep);
